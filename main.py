"""
Главное приложение Obsidian Clone
"""

import sys
from pathlib import Path
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QHBoxLayout,
                             QVBoxLayout, QSplitter, QMessageBox, QInputDialog,
                             QDialog, QPushButton)
...
class ObsidianClone(QMainWindow):
    def __init__(self):
        ...
        # Создаём заметку "Добро пожаловать" если нет
        if "Добро пожаловать" not in self.storage.notes:
            self.storage.save_note("Добро пожаловать",
                                   "# Салам Пополам! 🎉\n\n"
                                   "## 🎨 Возможности\n\n"
                                   ...
                                   "## 🎮 GameDev Tracker\n\n"
                                   "Нажмите на ✦ слева чтобы открыть GameDev Tracker для управления проектами!\n\n"
                                   "#tutorial #welcome")
    ...
    def is_welcome_note(self, title: str) -> bool:
        """Проверяет является ли заметка 'Добро пожаловать'"""
    ...
    def init_ui(self):
        ...
        # 1. Ribbon
        self.ribbon = Ribbon(self, is_gamedev_window=False)
    ...
    def init_shortcuts(self):
        ...
        QShortcut(QKeySequence("Ctrl+Shift+N"), self, self.on_create_today_daily)
    ...
    def on_file_selected(self, filename: str):
        if filename in self.storage.notes:
            ...
            # Обновляем цвет кнопки редактора
    ...
    def on_create_today_daily(self):
        from datetime import datetime
        today = datetime.now().strftime("%Y-%m-%d")
        self.on_daily_note_requested(today)
